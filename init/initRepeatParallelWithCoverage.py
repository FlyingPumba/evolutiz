...
def process_results(data):
	...
	if individual == False:
		...
		logger.log_progress("\rInit population in parallel: " + str(len(individuals_generated)) + "/" + str(total_individuals))

def initPop(device_manager, gen_individual_with_coverage_function, n, result_dir, apk_dir, package_name, gen):
	# init global states
	global total_individuals
	total_individuals = n
	...
	while len(individuals_generated) < total_individuals:
		...
		if remaining_individuals_to_generate > 0 and len(idle_devices) > 0:
			...
			pool.apply_async(gen_individual_with_coverage_function,
							 args=(device, result_dir, apk_dir, package_name, gen, remaining_individuals_to_generate),
