...
def upload_string_xml(device, decoded_dir, package_name):
    ...
    os.system(
        "$ANDROID_HOME/platform-tools/adb -s " + device + " shell rm /mnt/sdcard/" + package_name + "_strings.xml" + logger.redirect_string())
    os.system(
        "$ANDROID_HOME/platform-tools/adb -s " + device + " push " + string_xml_path + " /mnt/sdcard/" + package_name + "_strings.xml" + logger.redirect_string())
