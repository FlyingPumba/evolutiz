...
class Emulator(Device):
    ...
    def boot(self, port: Optional[int] = None, adb_port: Optional[int] = None) -> None:
        ...
        if not self.avd_manager.avd_name_exists(self.avd_name):
            raise Exception("AVD name {0} doesn't exist. Check that the provided AVD series ({1}) is correct."
                            .format(self.avd_name, self.avd_manager.avd_series))
        ...
        output, errors, result_code = run_cmd(adb.adb_cmd_prefix + " start-server",
