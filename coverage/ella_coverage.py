...
def get_ella_suite_coverage(scripts, device, apk_dir, package_name, gen):
    ...
    for index, script in enumerate(scripts):
        ...
        os.system(
            "$ANDROID_HOME/platform-tools/adb -s " + device + " push " + script + " /mnt/sdcard/" + " > " + std_out_file)
        script_name = script.split("/")[-1]
        # start motifcore replay
        # add timeout
        command = Command(
            "$ANDROID_HOME/platform-tools/adb -s " + device + " shell motifcore -p " + package_name + " --bugreport --throttle " + str(
                settings.THROTTLE) + " -f /mnt/sdcard/" + script_name + " 1")
        command.run(timeout=600)
        # print "... Start evaluating a script"
        # p = subprocess.Popen("$ANDROID_HOME/platform-tools/adb -s " + settings.DEVICE + " shell motifcore -p " + package_name + " --throttle " + str(settings.THROTTLE) + " -f /mnt/sdcard/" + script_name + " 1", shell=True, stdout=subprocess.PIPE).communicate()[0]
        # os.system("$ANDROID_HOME/platform-tools/adb -s " + device + " shell motifcore -p " + package_name + " --bugreport --throttle " + str(settings.THROTTLE) + " -f /mnt/sdcard/" + script_name + " 1" + " > " + std_out_file)
        ...
        # close app
        os.system(
            "$ANDROID_HOME/platform-tools/adb -s " + device + " shell pm clear " + package_name + " > " + std_out_file)
        os.system(
            "$ANDROID_HOME/platform-tools/adb -s " + device + " shell am force-stop " + package_name + " > " + std_out_file)
